...
export const ListeningScene: React.FC = () => {
  ...
  for (let i = 0; i < barCount; i++) {
    ...
    // Generate organic wave math: overlay multiple sine waves at different frequencies
  ...
  return (
    <AbsoluteFill style={{ backgroundColor: COLORS.bg, justifyContent: 'space-between', alignItems: 'center', overflow: 'hidden', padding: `${130 * scale}px ${80 * scale}px` }}>
      <OrbSet visible={true} />
      ...
      <div style={{ opacity: titleOpacity, transform: `translateY(${titleY}px)`, textAlign: 'center', zIndex: 1 }}>
        ...
        <h2 style={{ color: COLORS.white, fontSize: 40 * scale, fontWeight: 800, fontFamily: FONT.primary, letterSpacing: '-1.5px', lineHeight: 1.1 }}>
          No Typing. <br /> Just Speak Naturally.
        </h2>
      </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 36 * scale, opacity: waveOpacity, transform: `scale(${waveScale})`, zIndex: 1 }}>
        ...
        >
          ...
          <span style={{ color: COLORS.white, fontSize: 13 * scale, fontWeight: 600, fontFamily: FONT.primary, letterSpacing: '0.05em', textTransform: 'uppercase' }}>
            User Speaking...
          </span>
        </div>
      </div>
      ...
      >
        <p style={{ color: COLORS.zinc400, fontSize: 18 * scale, fontFamily: FONT.primary, lineHeight: 1.5, letterSpacing: '-0.1px' }}>
          Our advanced conversational model transcripts,
          <br />
          interprets, and evaluates your voice answer instantly.
        </p>
      </div>
    </AbsoluteFill>
  );
