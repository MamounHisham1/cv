    ...
    <div class="relative min-h-screen overflow-hidden bg-zinc-950 text-zinc-100">
        ...
        <div class="relative mx-auto max-w-7xl px-4 py-8 md:px-6 lg:px-8">
            ...
            <div class="mb-8 flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
                <div>
                    <p class="mb-1 text-sm font-semibold uppercase tracking-[0.2em] text-emerald-400">{{ __('Dashboard') }}</p>
                    <h1 class="text-3xl font-bold text-white md:text-4xl">
                        {{ __('Welcome back,') }} <span class="bg-gradient-to-r from-emerald-400 to-emerald-300 bg-clip-text text-transparent">{{ auth()->user()->name }}</span>!
                    </h1>
                    <p class="mt-1 text-sm text-zinc-400">{{ __("Ready to build your perfect CV? Let's get started.") }}</p>
                </div>
            ...
            <div class="mb-8 grid grid-cols-2 gap-4 lg:grid-cols-4">
                <div class="overflow-hidden rounded-2xl border border-white/10 bg-zinc-950/80 p-5 backdrop-blur-xl">
                    ...
                    <p class="text-xs text-zinc-500">{{ __('Total CVs') }}</p>
                </div>
                <div class="overflow-hidden rounded-2xl border border-white/10 bg-zinc-950/80 p-5 backdrop-blur-xl">
                    ...
                    <p class="text-2xl font-bold text-white">0</p>
                    <p class="text-xs text-zinc-500">{{ __('Views This Week') }}</p>
                </div>
                <div class="overflow-hidden rounded-2xl border border-white/10 bg-zinc-950/80 p-5 backdrop-blur-xl">
                    ...
                    <p class="text-2xl font-bold text-white">0</p>
                    <p class="text-xs text-zinc-500">{{ __('AI Suggestions') }}</p>
                </div>
                <div class="overflow-hidden rounded-2xl border border-white/10 bg-zinc-950/80 p-5 backdrop-blur-xl">
                    ...
                    <p class="text-xs text-zinc-500">{{ __('Time Saved') }}</p>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 gap-6 lg:grid-cols-3">
                ...
                <div class="lg:col-span-2">
                    <div class="overflow-hidden rounded-3xl border border-white/10 bg-zinc-950/80 p-6 backdrop-blur-xl">
                        <div class="mb-6 flex items-center justify-between">
                            <h2 class="text-lg font-bold text-white">{{ __('Recent CVs') }}</h2>
                            <a href="{{ route('cv.builder') }}" wire:navigate
                        ...
                        <div class="flex flex-col items-center justify-center py-14 text-center">
                            ...
                            <h3 class="mb-2 font-semibold text-white">{{ __('No CVs Yet') }}</h3>
                            <p class="mb-6 max-w-sm text-sm leading-relaxed text-zinc-400">
                                {{ __('Start building your professional CV with our AI-powered builder. Create ATS-optimized resumes that get you noticed.') }}
                            </p>
                            <a href="{{ route('cv.builder') }}" wire:navigate
                ...
                <div class="space-y-6">
                    ...
                    {{-- Quick Actions --}}
                    <div class="overflow-hidden rounded-3xl border border-white/10 bg-zinc-950/80 p-6 backdrop-blur-xl">
                        <h2 class="mb-5 text-lg font-bold text-white">{{ __('Quick Actions') }}</h2>
                        <div class="space-y-2">
                            <a href="{{ route('cv.builder') }}" wire:navigate
                            ...
                            <a href="{{ route('cv.evaluator') }}" wire:navigate
                               class="flex items-center gap-3 rounded-2xl border border-emerald-400/20 bg-emerald-500/10 px-4 py-3 text-sm font-medium text-emerald-300 transition-all duration-200 hover:bg-emerald-500/15 hover:text-emerald-200">
                                ...
                                {{ __('AI CV Evaluator') }}
                                <span class="ml-auto rounded-full border border-emerald-400/30 bg-emerald-500/20 px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wider text-emerald-300">{{ __('New') }}</span>
                            </a>
                            <a href="{{ route('cv.builder') }}" wire:navigate
                               class="flex items-center gap-3 rounded-2xl border border-white/5 bg-white/5 px-4 py-3 text-sm font-medium text-zinc-300 transition-all duration-200 hover:bg-white/10 hover:text-white">
                    ...
                    <div class="overflow-hidden rounded-3xl border border-white/10 bg-zinc-950/80 p-6 backdrop-blur-xl">
                        <div class="mb-3 flex items-center gap-3">
                            ...
                            <h3 class="font-bold text-white">{{ __('Pro Tip') }}</h3>
                        </div>
                        <p class="mb-4 text-sm leading-relaxed text-zinc-400">
                            {{ __('Tailor your CV for each job application. Our AI can help identify the right keywords and optimize your resume for Applicant Tracking Systems (ATS).') }}
                        </p>
