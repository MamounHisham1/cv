        ...
        @switch($sectionKey)
            @case('experience')
                @if($cv->experiences->count() > 0)
                    <section class="mb-6">
                        <h2 class="text-lg font-bold uppercase border-b border-gray-400 mb-3"> {{ __('Professional Experience') }} </h2>
            ...
            @case('education')
                @if($cv->educations->count() > 0)
                    <section class="mb-6">
                        <h2 class="text-lg font-bold uppercase border-b border-gray-400 mb-3"> {{ __('Education') }} </h2>
                        @foreach($cv->educations as $edu)
                            <div class="mb-2">
                                <div class="flex justify-between">
                                    <span class="font-bold">{{ $edu->institution }}</span>
                                    <span class="text-sm text-gray-600">{{ $edu->start_date?->format('Y') }} - {{ $edu->is_current ? __("Present") : $edu->end_date?->format('Y') }}</span>
                                </div>
            ...
            @case('skills')
                @if($cv->skills->count() > 0)
                    <section class="mb-6">
                        <h2 class="text-lg font-bold uppercase border-b border-gray-400 mb-3"> {{ __('Skills') }} </h2>
            ...
            @case('certifications')
                @if($cv->certifications->count() > 0)
                    <section class="mb-6">
                        <h2 class="text-lg font-bold uppercase border-b border-gray-400 mb-3"> {{ __('Certifications') }} </h2>
            ...
            @case('projects')
                @if($cv->projects->count() > 0)
                    <section class="mb-6">
                        <h2 class="text-lg font-bold uppercase border-b border-gray-400 mb-3"> {{ __('Projects') }} </h2>
            ...
            @case('languages')
                @if($cv->languages->count() > 0)
                    <section class="mb-6">
                        <h2 class="text-lg font-bold uppercase border-b border-gray-400 mb-3"> {{ __('Languages') }} </h2>
                        <div class="flex flex-wrap gap-4">
                            @foreach($cv->languages as $lang)
                                <span class="text-sm"><strong>{{ $lang->language }}</strong> - {{ ucfirst(__($lang->proficiency)) }}</span>
                            @endforeach
                        </div>
                    </section>
                @endif
