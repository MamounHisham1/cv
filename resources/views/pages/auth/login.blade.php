<x-layouts::auth :title="__('Log in')">
    ...
    <div class="flex flex-col gap-8 text-zinc-200">
        ...
        <div class="text-center animate-slide-up">
            <div class="mb-4 inline-flex items-center rounded-full border border-emerald-400/20 bg-emerald-500/10 px-3 py-1 text-xs font-semibold uppercase tracking-[0.24em] text-emerald-200">{{ __("Design 4 Experience") }}</div>

            <h1 class="mb-2 text-3xl font-bold text-white">{{ __("Welcome Back") }}</h1>
            <p class="text-sm text-zinc-400">{{ __("Sign in to continue building your professional CV") }}</p>
        </div>
        ...
        <div class="relative my-4">
            ...
            <div class="relative flex justify-center text-xs uppercase">
                <span class="bg-zinc-950/80 px-4 text-zinc-500">{{ __("Or continue with") }}</span>
            </div>
        </div>
        ...
        <div class="animate-slide-up delay-200">
            <x-ui::link :href="route('auth.google.redirect')" class="{{ $secondaryButtonClasses }} inline-flex items-center justify-center rounded-lg">
                ...
                </svg>{{ __("Sign in with Google") }}</x-ui::link>
        </div>
