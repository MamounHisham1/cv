        ...
        @if($showForm)
            <form wire:submit="saveSkill" class="space-y-4 mb-8 form-section">
                ...
                <flux:field>
                    ...
                    <flux:input wire:model="form.name" placeholder="e.g., Python, React, Project Management" required />
                </flux:field>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <flux:field>
                        ...
                        <flux:select wire:model="form.level">
                ...
                @if(isset($skills[$categoryKey]) && count($skills[$categoryKey]) > 0)
                    <div>
                        <h4 class="text-sm font-bold text-zinc-900 dark:text-white mb-4 flex items-center gap-2">
                            <flux:icon name="{{ $categoryIcons[$categoryKey] ?? 'check' }}" class="w-4 h-4 text-emerald-500" />
                            {{ $categoryLabel }}
                                ...
                                <div class="group relative bg-zinc-50 dark:bg-zinc-800/50 rounded-xl p-4 transition-all duration-200 hover:shadow-sm border border-transparent hover:border-emerald-200 dark:hover:border-emerald-800">
                                    <div class="flex items-start justify-between gap-3">
                                        ...
                                        <div class="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity duration-200">
                                            <flux:button variant="ghost" size="xs" wire:click="editSkill({{ $skill['id'] }})" icon="pencil" class="shrink-0" />
                                            <flux:button variant="ghost" size="xs" wire:click="deleteSkill({{ $skill['id'] }})" wire:confirm="Delete this skill?" icon="trash" class="shrink-0 text-red-600 hover:text-red-700" />
                                        </div>
                                    </div>
                                </div>
                            @endforeach
        ...
        @if(empty($skills))
            <div class="text-center py-12">
                <div class="w-16 h-16 rounded-full bg-zinc-100 dark:bg-zinc-800 mx-auto mb-4 flex items-center justify-center">
                    <flux:icon name="bolt" class="w-8 h-8 text-zinc-400" />
                </div>
                <flux:heading size="md" class="mb-2">No Skills Added Yet</flux:heading>
                <p class="text-sm text-zinc-600 dark:text-zinc-400 mb-4">Start showcasing your technical and soft skills</p>
                <flux:button wire:click="addSkill" variant="primary" icon="plus">
